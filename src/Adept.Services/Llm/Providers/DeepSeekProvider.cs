...
{
    /// <summary>
    /// Provider for DeepSeek LLM services
    /// </summary>
    public class DeepSeekProvider : ILlmProvider
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            try
            {
                ...
                _apiKey = await _secureStorageService.RetrieveSecureValueAsync("deepseek_api_key") ?? string.Empty;
                _isInitialized = true;
                _logger.LogInformation("DeepSeek provider initialized");
            ...
            {
                _logger.LogError(ex, "Error initializing DeepSeek provider");
            }
        }

        /// <summary>
        /// Fetches the latest available models from the DeepSeek API
        /// </summary>
        ...
        public async Task<IEnumerable<LlmModel>> FetchAvailableModelsAsync()
        {
            if (!HasValidApiKey)
            {
                _logger.LogWarning("Cannot fetch DeepSeek models: API key not set");
                return _availableModels;
            }

            try
            {
                // DeepSeek doesn't have a public models endpoint, so we'll return the hardcoded list
                ...
                _logger.LogInformation("Fetched {Count} DeepSeek models", _availableModels.Count);
                return _availableModels;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error fetching DeepSeek models");
                return _availableModels;
            }
        }
        ...
        public async Task SetApiKeyAsync(string apiKey)
        {
            ...
            {
                _apiKey = apiKey;
                await _secureStorageService.StoreSecureValueAsync("deepseek_api_key", apiKey);
                _logger.LogInformation("DeepSeek API key set");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error setting DeepSeek API key");
            }
        }
        ...
        public Task<bool> SetModelAsync(string modelId)
        {
            ...
            {
                _currentModel = model;
                _logger.LogInformation("DeepSeek model set to {ModelName}", model.Name);
                return Task.FromResult(true);
            }

            _logger.LogWarning("DeepSeek model {ModelId} not found", modelId);
        ...
        public async Task<LlmResponse> SendMessagesAsync(IEnumerable<LlmMessage> messages, string? systemPrompt = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("DeepSeek response received");
                return llmResponse;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error sending message to DeepSeek");
                throw;
            }
        }
        ...
        private async Task<LlmResponse> SendMessagesStreamingAsync(IEnumerable<LlmMessage> messages, Action<string> onChunk, string? systemPrompt = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("DeepSeek streaming response completed");
                return llmResponse;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error sending streaming message to DeepSeek");
                throw;
            }
        }
        ...
        public async Task<LlmResponse> SendMessagesWithToolsAsync(IEnumerable<LlmMessage> messages, IEnumerable<LlmTool> tools, string? systemPrompt = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("DeepSeek response with tools received");
                return llmResponse;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error sending message with tools to DeepSeek");
                throw;
            }
        }
    }
